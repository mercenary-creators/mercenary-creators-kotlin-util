...
abstract class AbstractMutableMetaData<T : Any> : LinkedHashMap<String, T>, MutableMetaDataType<T> {
    ...
    protected constructor(vararg args: Pair<String, T>) : super(make(*args))
    ...
    private val tran = MetaDataTransformers<String, T>()

    override val transformers: MetaDataTransformers<String, T>
        get() = tran

    fun padding(): String = pads

    fun padding(many: Int) {
        pads = " ".repeat(max(many, 0))
    }
    ...
    protected companion object {
        ...
        fun <T : Any> make(vararg args: Pair<String, T>): Map<String, T> {
            return if (args.isEmpty()) emptyMap()
            else LinkedHashMap<String, T>(args.size).also {
                for ((k, v) in args) it[k] = v
            }
